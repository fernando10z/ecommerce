...
// Consulta los datos de diseño personalizados para la sección de mujeres limitando a un solo registro
$sqlDiseno = "SELECT * FROM web_design_women WHERE id = 1 LIMIT 1";
$stmtD = $conn->query($sqlDiseno);
$diseno = $stmtD->fetch(PDO::FETCH_ASSOC);

// Asigna valores predeterminados para el banner principal en caso de no tener información en la base de datos
...
try {
    // Obtiene productos activos y visibles (en tienda o solo catálogo) para la categoría mujer
    $sql = "SELECT p.id, p.name, p.base_price, p.sale_price, pi.url as image
            ...
            WHERE (c.name = 'Mujer' OR c.slug IN ('mujer', 'women')) 
              AND p.status = 'active' 
        ...
        <div class="products-grid">
            <?php if (!empty($productos)): ?>
                <?php foreach ($productos as $producto): 
                    ...
                    // Verifica si hay un precio de oferta válido y menor al precio base para determinar el precio a mostrar
